const updateCurrentMailout = async (mailout) => {
  const sendURL = `./API/set_current_mailout_tssc.php?mailout=${mailout}`;
  emailsSent.appendChild(loading);
  const res = await fetch(sendURL);
  const output = await res.text();
  const status = output === 'SUCCESS' ? `The Sad Song Co. Mailout ${mailout} has started sending` : `There was an error, please try again`;
...
const populateMailoutSelect = async () => {
  const dataURL = './API/mailoutOptions_tssc.php';
